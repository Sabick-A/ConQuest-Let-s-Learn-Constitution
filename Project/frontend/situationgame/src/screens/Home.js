...
import homepagebgm from '../music/home page bgm.mp3'
import { VolumeSlash, VolumeHigh } from 'iconsax-react';
import Modal from 'react-modal';
import home from '../images/Home.jpeg'
import click from '../music/Click - Sound Effect (HD).mp3'
function Home() {
    ...
    useEffect(() => {
        const audio = new Audio(homepagebgm);
        audio.loop = true;
        audio.muted = isMuted;
        audio.play().catch((error) => console.error('Autoplay prevented:', error));
        audioRef.current = audio;

        // Cleanup on component unmount or when `isMuted` changes
        return () => {
            if (audioRef.current) {
    ...
    const startGame = () => {
        navigate('/OpeningWell');
    };

    const clicked = () => {
        const audio = audioRef.current || new Audio(homepagebgm);
    ...
    return (
        <div className="h-screen bg-cover flex flex-col " style={{ backgroundImage: `url(${home})` }}>
            <div className='flex h-1/4 items-center justify-center mt-60'>
                <h1 className="font-serif text-7xl text-white cursor-pointer sm:text-9xl sm:text-white sm:hover:animate-pulse sm:cursor-pointer"
                    >The Game of Justice</h1>
            </div>

            <div className='flex h-1/3 items-center justify-center mt-34'>
                <h1 onClick={startGame} className="font-serif text-7xl text-white cursor-pointer sm:text-7xl sm:text-white sm:hover:animate-pulse m:hover:duration-10000sm:cursor-pointer">Start Game
                </h1>
            </div>
            ...
            <div className='absolute bottom-0 left-0 ml-5 mb-5 max-w-64 sm:ml-10 sm:mb-5 sm:max-w-max'>
                ...
                <h1 className='font-serif text-base text-slate-300 max-w-64 mt-2 sm:font-serif sm:text-2xl sm:text-slate-300 sm:max-w-max sm:mt-2'>
                    Note: Only hit continue if the text hasn't changed after three seconds.
                </h1>
            </div>
